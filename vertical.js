...
const Text = {
    ...
    draw: function (str = '') {
        ...
        let displayText = str === '' ? this.text : str;
        ctx.fillText(displayText, (CANVAS_WIDTH / 2) - 150, CANVAS_HEIGHT / 2 + 60);
    }
};
    ...
    move: function () {
        ...
        for (let i = 0; i < LOC.length; i++) {
            if (this.x >= LOC[i][0] && this.x <= LOC[i][0] + OBSTACLE_WIDTH &&
                this.y - this.radius >= LOC[i][1] && this.y - this.radius <= LOC[i][1] + OBSTACLE_HEIGHT) {
...
const paddle = {
    ...
    move: function (e) {
        if (GAME_ORIENTATION === 'horizontal') {
            ...
            this.x = canvas.width - PADDLE_HEIGHT; 
        }
        X = this.x;
        Y = this.y;
    }
